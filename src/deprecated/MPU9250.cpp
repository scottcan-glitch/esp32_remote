...
/* MPU9250_MS5637_ESP32 Basic Example Code
 ...
 4K7 resistors are the MPU9250+MS5637 breakout board.
...
#define PI_TX 17 
#define PI_RX 18
HardwareSerial PiSerial(1); // Use Hardware Serial 1 for the Pi link
...
void setup()
{
  ...
  // High speed link to Pi (115200 or 921600 for even lower latency)
...
{  
   ...
   //   roll  *= 180.0f / PI;
    ...
    if(SerialDebug) {
      ...
      // --- PACKAGING DATA FOR SERIAL LINK ---
      val_array[0] = q[0];
      val_array[1] = q[1];
      val_array[2] = q[2];
      val_array[3] = q[3];
      ...
      val_array[5] = ay;
      val_array[6] = az;
      val_array[7] = (gx * PI/180.0f);
      val_array[8] = (gy * PI/180.0f);
      val_array[9] = (gz * PI/180.0f);
      val_array[10] = mx;
      val_array[11] = my;
      val_array[12] = mz;
      val_array[13] = (9.*(float)Temperature/5. + 32.); // Temp F
      val_array[14] = (float)Pressure;
      val_array[15] = (float)sumCount/sum; // Filter Rate
      val_array[16] = yaw;
      val_array[17] = altitude;

      // --- SENDING TO RASPBERRY PI ---
      // This creates a Comma Separated Variable string
      for(int i = 0; i < 18; i++) {
        PiSerial.print(val_array[i], 4); // 4 decimal places
        if(i < 17) PiSerial.print(",");   // Add comma between values
      }
      PiSerial.println(); // Send Newline to signal end of frame

      // Signal activity on the LED
